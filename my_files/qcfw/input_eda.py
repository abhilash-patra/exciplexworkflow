...
from qcfw.utils import lower_and_check_unique, read_pattern, read_table_pattern
...
class QCInputEDA(MSONable):
    ...
    def __init__(
        ...
        self.frgm_cis_n_roots = lower_and_check_unique(frgm_cis_n_roots)
    ...
    def __str__(self):
        ...
        if self.van_der_waals:
            ...
            combined_list.append("")
        # section for fragment section required for EX_EDA ******************************
        if self.frgm_cis_n_roots:
            combined_list.append(self.frgm_cis_n_roots_template(self.frgm_cis_n_roots))
            combined_list.append("")
        #********************************************************************************
        # plots section
        if self.plots:
...
#******************************************************
        if "frgm_cis_n_roots" in sections:
            frgm_cis_n_roots = cls.read_frgm_cis_n_roots(string)
#*******************************************************
        return cls(
            molecule,
            rem,
...
#######################################For EXEDA-------------------------------------
    ...
    def molecule_template(molecule: Union[Molecule, Literal["read"]]) -> str:
        ...
        if isinstance(molecule, str):
            ...
            mol_list.append('--')
            mol_list.append(f" {int(molecule.charge)} {molecule.spin_multiplicity}")
            for site in molecule.sites[22:55]:
                mol_list.append(
            ...
            mol_list.append('--')
            mol_list.append(f" {int(molecule.charge)} {molecule.spin_multiplicity}")
            for site in molecule.sites[0:22]:
                mol_list.append(
                    " {atom}     {x: .10f}     {y: .10f}     {z: .10f}".format(
                        atom=site.species_string, x=site.x, y=site.y, z=site.z
                    )
...
###############################################EX_EDA**************************
    @staticmethod
    def frgm_cis_n_roots_template(frgm_cis_n_roots: Dict) -> str:
        """
        Args:
            rem ():
        ...
        """
        rem_list = []
        rem_list.append("$frgm_cis_n_roots")
        for key, value in frgm_cis_n_roots.items():
...
#************************************************
    def read_frgm_cis_n_roots(string: str) -> Dict:
        """
        Parse frgm_cis_n_roots from string.
