...
class QCInputCH(MSONable):
    ...
    def molecule_template(molecule: Union[Molecule, Literal["read"]]) -> str:
        ...
        if isinstance(molecule, str):
            ...
            mol_list.append("0 1")
            for site in molecule.sites[1:2]:
#                mol_list.append(
#                    " {atom}     {x: .10f}     {y: .10f}     {z: .10f}".format(
#                        atom=site.species_string, x=site.x, y=site.y, z=site.z
#                    )
#                )
                first_atom_positive_x = site.x > 0

            for site in molecule.sites[4:]:
                if (first_atom_positive_x and site.x > 0) or (not first_atom_positive_x and site.x < 0):
                   mol_list.append(
                       " {atom}     {x: .10f}     {y: .10f}     {z: .10f}".format(
                           atom=site.species_string, x=site.x, y=site.y, z=site.z
                       )
