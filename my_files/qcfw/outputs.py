...
#        header_pattern = r"\s*Standard Nuclear Orientation \(Angstroms\)\s+I\s+Atom\s+X\s+Y\s+Z"
#        table_pattern = r"\s+\d+\s+\w+\s+([\d\-\.]+)\s+([\d\-\.]+)\s+([\d\-\.]+)"
#        footer_pattern = r"\s+-{64}"
#
#        parsed_geometries = read_table_pattern(self.text, header_pattern, table_pattern, footer_pattern)
...
#                geoms.append(process_parsed_coords(parsed_geometry))
#----AP
#        if not any(parsed_geometries):
#           print("didnt parse")
#           alternative_header_pattern = r"\s+Standard Nuclear Orientation \(Angstroms\)"
#           alternative_table_pattern = r"\s+\d+\s+\w+\s+([\d\-\.]+)\s+([\d\-\.]+)\s+([\d\-\.]+)"
#           alternative_footer_pattern = r"[-]+\s*"
#
#           parsed_alternative_geometries = read_table_pattern(self.text, alternative_header_pattern, alternative_table_pattern, alternative_footer_pattern)
#
#           for jj, parsed_alternative_geometry in enumerate(parsed_alternative_geometries):
#               if not parsed_alternative_geometry:
#                  geoms.append(None)
#               else:
#                   geoms.append(process_parsed_coords(parsed_alternative_geometry))
#
#----
